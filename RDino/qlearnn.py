...
FC_LAYER_1_SIZE       = 960 # input to fully connected layer
FC_LAYER_2_SIZE       = 512 # fully connected layer
OUT_LAYER_SIZE        = ACTIONS
#======== Network
...
def createNetwork():
    # network weights
    W_conv1 = weight_variable([LAYER_1_FILTER_SIZE, LAYER_1_FILTER_SIZE, LAYER_1_FRAMES_COUNT, LAYER_1_FILTER_COUNT])
    b_conv1 = bias_variable([LAYER_1_FILTER_COUNT])

    W_conv2 = weight_variable([LAYER_2_FILTER_SIZE, LAYER_2_FILTER_SIZE, LAYER_2_FRAMES_COUNT, LAYER_2_FILTER_COUNT])
    b_conv2 = bias_variable([LAYER_2_FILTER_COUNT])
 
    W_conv3 = weight_variable([LAYER_3_FILTER_SIZE, LAYER_3_FILTER_SIZE, LAYER_3_FRAMES_COUNT, LAYER_3_FILTER_COUNT])
    ...
    # input layer
    s = tf.placeholder("float", [None, INPUT_SIZE_HEIGHT, INPUT_SIZE_WIDTH, LAYER_1_FRAMES_COUNT])
    ...
    h_conv3_flat = tf.reshape(h_pool3, [-1, FC_LAYER_1_SIZE])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    stateid = 0
    x_t, r_0, terminal, stateid = get_a_frame(do_nothing, stateid, None, prev_action)
    x_t1 = x_t
    ...
    while True:
        ...
        if (t % frame_per_sec) == 0:
            ...
            x_t1, r_t, terminal, stateid = get_a_frame(a_t, stateid, x_t1, prev_action)
            x_t1 = np.reshape(x_t1, (160, 640, 1))
...
def process_a_frame(gamesec, fail_section):
    ...
    game_section = cv2.resize(gray_screen, (640, 160))
...
def get_a_frame(action, stateid, prev_frame, prev_action):
    reward = 0.1
    ra = "_p_"
    take_action(action)
    ...
    if terminal:
        ...
        jump()# reset
    #if save_frames:
    #save_frames(prev_frame, prev_action, stateid, ra)

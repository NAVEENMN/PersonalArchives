...
class Actor():
  ...
  def get_loss(self, pi_st, rt_st, v_st):
    A = rt_st - v_st
    h_pi = tf.reduce_sum(pi_st*tf.log(pi_st))
    with tf.name_scope('AC_Loss'):
      value_loss = tf.losses.mean_squared_error(predictions=A, labels=rt_st)
      policy_loss = (-tf.log(pi_st)*A)-(self.beta*h_pi)
...
class reward_predictor():
  ...
  def predict_reward(self, phi_st, reuse=False):
    ...
    with tf.variable_scope('Reward_FcNet', reuse=reuse):
      reward = tf.layers.dense(phi_st, 512/2, name=n+"fc1")
      reward = tf.layers.dense(reward, 512, name=n+"fc2")
      return reward
  def get_loss(self, source, target):
    target = tf.multiply(tf.ones([1, 512]), target)

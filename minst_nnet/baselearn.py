...
class layers:
	def __init__(self, layer_id, input_connections, output_connections, num_of_neurons):
		...
		self.weights = tf.Variable(tf.zeros([input_connections, output_connections]))
		wname = self.layer_name+"_weights"
		bname = self.layer_name+"_bias"
		self.weights = tf.Variable(tf.truncated_normal([input_connections, output_connections]), name=wname)
		self.bias = tf.Variable(tf.truncated_normal([1, num_of_neurons]), name=bname)
	...
	def feed_forward(self, x, layer_id):
		...
		for layer_id in range(0, num_of_layers):
			...
			with tf.name_scope(layer_name):
				layer_net = tf.matmul(x, weights)
				node_values.append(layer_net)
				x = self.sigmoid(layer_net)
				pv("weights", weights)
				pv("x", x)
		errors = tf.subtract(tc, x)
...
def get_network_descpription():
	...
	LAYERS = list()
	for x in range(1, num_of_layers+1):
		l = dict()

...
def batch_generator1(data_dir, images, actions, batch_size, is_training):
    """
    Generate training image give image paths and associated steering angles
    """
    imgs = list()#np.empty([batch_size, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_CHANNELS])
    acts = list()#np.empty(batch_size)
    images, actions = shuffle(images, actions)
    for x in range(0, batch_size):
        imgs.append(images[x])
        acts.append(actions[x])
    print(np.vstack(imgs).shape)
    print( np.vstack(acts).shape)
    yield np.vstack(imgs), np.vstack(acts)

...
'''
To do:
1) edit state theta and if we can move pend
2) try td learning by random sampling
3) different reward instead of given one.
4) increase num of workers
5) try GPU version.
'''
class GameEnv:
    def __init__(self):
...
class ActorCritic:
    ...
    def push_global_weights(self, feed_dict):
        self.sess.run([self.update_a_op, self.update_c_op], feed_dict=feed_dict)
    ...
    def train_sample_from_memory(self, memory):
        state, action, target_vs = memory.sample_batch(20)
        st = np.reshape(state, [-1, state_space])
        at = np.reshape(state, [-1, action_space])
        rt = np.reshape(target_vs, [-1, 1])
        feed_dict = {self.state_ph: st,
                     self.action_ph: at,
                     self.value_target: rt}
        self.sess.run([self.update_c_op], feed_dict=feed_dict)
...
# global worker has access to global policy parameters
class global_worker:
        ...
        while not self.coord.should_stop() and global_episodes < MAX_GLOBAL_EP:
            ...
            for ep_t in range(MAX_EP_STEP):
                ...
                if total_step % UPDATE_GLOBAL_ITER == 0 or done:
                    v_s_ = 0 if done else self.ac.get_value(st1)
                    ...
                    for i in range(0, len(buffer_s)):
                        self.memory.add((buffer_s[i], buffer_a[i], buffer_v_target[i]))

                    buffer_s = np.reshape(buffer_s, [-1, self.env.state_space])
                    buffer_v_target = np.reshape(buffer_v_target, [-1, self.env.reward_space])

                    feed_dict = {self.ac.state_ph: buffer_s,
                                 self.ac.action_ph: buffer_a,
                                 self.ac.value_target: buffer_v_target}

                    self.ac.push_global_weights(feed_dict)
                    if global_episodes > 10:
                        self.ac.train_sample_from_memory(self.memory)
                    buffer_s, buffer_a, buffer_r = [], [], []
                    self.ac.pull_weights()
    ...
    def work(self, saver):
        ...
        while not self.coord.should_stop() and global_episodes < MAX_GLOBAL_EP:
            ...
            for ep_t in range(MAX_EP_STEP):
                ...
                if total_step % UPDATE_GLOBAL_ITER == 0 or done:
                    v_s_ = 0 if done else self.local_ac.get_value(st1)
...
class Controller:
    def __init__(self, sess, mode):
        ...
        with tf.device("/cpu:0"):
            ...
            workers = []
            # global thread
            workers.append(global_worker("global", self.global_ac, coord, sess))
        ...
        while not done:
            ...
            print("theta dot {} {}, at {} r {}".format(np.arccos(st1[0])*np.arcsin(st1[1]), st1[2], at, rt))
...
class ReplayBuffer(object):
    ...
    def sample_batch(self, sample_size):
        ...
        values = np.reshape([_[1] for _ in batch], [-1, 1])

...
def trainNetwork(sess, X, TRAIN=True):
	...
	d_on_generated = tf.reduce_mean(discriminator(generator(batch_size, z), keep_prob))
	...
	images_for_tensorboard = generator(batch_size, z)
	...
	if (TRAIN == False):
		...
		for x in range(0, 100):
			mean = 0+x/200
			sd = 0.05+x/1000 #standard deviation
			z = np.float32(np.random.normal(mean, sd, batch_size*ENCODER_LATENT_DIM))
			z = np.reshape(z,[batch_size, ENCODER_LATENT_DIM])
			image = generator(batch_size, z)
			image = image.eval()
			visulize_a_image(image)
			#images.append(np.reshape(image, (IMAGE_WIDTH, IMAGE_HEIGHT, IMAGE_CHANNEL)))
		#imageio.mimsave('images\\2.gif', images) #gif generate
		return
	...
	while True:
		...
		if dLossFake > 0.6:
			...
			_, vaeLoss, dLossReal, dLossFake, gLoss = sess.run([d_trainer_fake, vae_loss, d_loss_real, d_loss_fake, g_loss],

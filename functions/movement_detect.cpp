...
int main(int argc, const char * argv[]) {
    ...
    for(;;)
    {
        Mat frame1, frame2;
        Mat f1, f2;
        cap.set(CV_CAP_PROP_POS_MSEC,10);
        cap >> frame1;
        f1 = frame1.clone();
        cap.set(CV_CAP_PROP_POS_MSEC,5000);
        cap>> frame2;
        f2 = frame2.clone();
        if( f1.empty() || f2.empty()) break; // end of video stream
        Mat difference = f1 - f2;
        //imshow("difference", difference);
        cvtColor(f1, f1, COLOR_BGR2GRAY);
        cvtColor(f2, f2, COLOR_BGR2GRAY);
        goodFeaturesToTrack(f1,features,100,0.01,0.01, f2,3,false, 0.04);//extract features to track
        cout<<features.size()<<endl;
        for(int i=0; i<features.size(); i++){
            cout<<features[i]<<endl;
        }
        calcOpticalFlowPyrLK(f1,f2,features,track,status,err);//optical flow
        for(int i=0; i<features.size(); i++){
            //cout<<features[i]<<"::"<<track[i]<<endl;
            DrawLine(difference, features[i], track[i]);
        }
        imshow("difference", difference);
